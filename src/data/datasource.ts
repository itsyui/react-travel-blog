...
class DataSource implements IDataSourceLake {
    async getAll(type: string, params: any) {
        try {
            let records = data;
            if (params.maxItems !== undefined && params.skipCount !== undefined) {
                records = data.slice(params.skipCount, ((params.skipCount / params.maxItems) + 1) * params.maxItems);
            }
            records["totalRecordsCount"] = data.length;
            return records;
        } catch (e) {
            console.log(e);
        }
